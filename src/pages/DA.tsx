...
export default function DA(){
    const [materia, setMateria] = useState<Modulo[]>(() => {
        ...
        const initialMaterias = savedMaterias ? JSON.parse(savedMaterias) : [
            ...
            {title: 'Musicas em Ingles', url: music, link:'http://localhost:5173/musicas'},
    ...
    const adicionarModulo = (novoModulo: {title: string; url: string; background: string}) => {
        ...
        if(existingModulo) {
            alert("Uma materia com este título já exite. Escolha outro título!");
            return;
        }
    ...
    return (
        <>
            <div className="body-mat-da">
                <header className="custom-header-da">
                    <h1>Pagina de Inclusão</h1>
                </header>
                <div className="container-da">
                    {materia.map((item, index) => (
                        <div key={item.title} className="image-container-da">
                            <a href={item.link}>
